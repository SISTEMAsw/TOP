    ...
    def get_data(self):
        ...
        if self.masking == True:
            ...
            if self.mgrs_tile != 'None':
                url = "https://{}/wcs?service=WCS&Request=GetCoverage&version=2.0.0&subset=unix({})&format=image/tiff&filter=false&CoverageId={}&subset=Lat({},{})&subset=Long({},{})&token={}&scale={}&mgrs_tile={}".format(self.endpoint,self.time_t,self.collection, geom.bounds[1], geom.bounds[3], geom.bounds[0], geom.bounds[2], self.token, self.scale,self.mgrs_tile)
            else:
...
class getTimeSeries:
    def __init__(self, endpoint, collection, time_t, lat, long, token = 'None'):
        self.endpoint = endpoint
        self.collection = collection
        self.time_t = time_t
        self.lat = lat
    ...
    def get_data(self):
        url = 'https://{}/wcs?service=WCS&Request=GetCoverage&version=2.0.0&subset=unix({})&format=application/xml&CoverageId={}&subset=Lat({})&subset=Long({})&filter=false&token={}'.format(self.endpoint, self.time_t, self.collection, self.lat, self.long, self.token)
        print(url)
        result = requests.get(url)
        xml_data = result.text
